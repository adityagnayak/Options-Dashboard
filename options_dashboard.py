...
with st.sidebar:
    ...
    if st.session_state.apply_iv and st.session_state.iv_result:
        ...
        if not np.isnan(iv_val):
            vol_pct_default = int(iv_val * 100)
            st.session_state.apply_iv = False

    vol_pct = st.slider("Volatility (%)", min_value=1, max_value=200,
                        value=vol_pct_default, key="vol_slider")
    sigma = vol_pct / 100.0
    ...
    with col3:
        ...
        fig_bar.update_layout(
            ...
            **PLOTLY_LAYOUT,
            ...
            xaxis=dict(gridcolor="#2A3347", zerolinecolor="#2A3347"),
            yaxis=dict(gridcolor="rgba(0,0,0,0)", showticklabels=False),
        )
        st.plotly_chart(fig_bar, use_container_width=True)

    with col4:
        mnm_lbl, mnm_cls2, mnm_pct2 = moneyness_label(S, K, option_type)
        pct_dist = (S / K - 1) * 100
        st.markdown(f"""
